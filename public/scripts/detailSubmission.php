...
$id = $_SESSION["id"];
if(!$id) {
    $unencodedArray = ['resp' => 'invalid', 'reason' => 'not logged in'];
...
if(strlen($name) == 0 || strlen($address) == 0 || strlen($city) == 0 || strlen($prov) == 0 ) {
    $unencodedArray = ['resp' => 'invalid', 'reason' => 'address not correct'];
    echo json_encode($unencodedArray);
    return;
}
...
if(!$postalValid) {
    $unencodedArray = ['resp' => 'invalid', 'reason' => 'postal not correct'];
    echo json_encode($unencodedArray);
    return;
}

if(!$comment) {
    $unencodedArray = ['resp' => 'invalid', 'reason' => 'no review comment'];
    echo json_encode($unencodedArray);
    return;
}
